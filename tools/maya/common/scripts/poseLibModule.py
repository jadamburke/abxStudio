...
def writePose(poseFile=None):
    ...
    for i in xrange(0, len(controls)):
        ...
        if showType[1] == "blendShape":
            ...
            attribute_element.setAttribute('value', str(mel.eval('blendShape -q -envelope ' + control)))

            listOfTargets = mel.eval('aliasAttr -q ' + control)
            ...
            for targetTmp in listOfTargets:
                if targetTmp.find('[') == -1:
                    ...
                    attribute_element.setAttribute('value', str(mel.eval('getAttr ' + control + "." + targetTmp)))

        # Are we dealing with a character set?
        elif showType[1] == "character":
            objectsPlusAttributes = mel.eval('sets -q ' + control)
